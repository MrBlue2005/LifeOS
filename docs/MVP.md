...
**Phase 2.10C Checkpoint 2 implementation:** `20260910170000_add_buy_later_reminder_claims.sql`, `20260910180000_fix_buy_later_reminder_claim_ambiguity.sql`, and `20260910190000_fix_buy_later_reminder_conflict_target.sql` provide an atomic, service-only claim function and are applied to the linked remote project. The claim RPC passed service-role execution, idempotency, distinct-subscription identity, ineligibility, and access-boundary validation. An hourly Supabase Cron job is documented to call a protected Node.js Vercel route using a Vault-held scheduler secret. The route processes a bounded batch, applies each stored IANA timezone at or after 09:00 local, and records one durable `sent`, `failed`, or `revoked` result for each `(item, reconsideration date, subscription, web_push)` identity. Automatic rollout is gated by a configured rollout date, so pre-existing overdue items are not backfilled. Scheduler-route deployment, Vault/Cron activation, and real-device automatic-delivery QA remain pending. There is no custom notification time, email channel, notification center, badge, retry queue, analytics, or digest.
