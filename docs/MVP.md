...
**Phase 2.10C Checkpoint 1 implementation:** server-only VAPID configuration, a service-role client used only by notification delivery, a safe payload contract, and an authenticated manual test-send action are implemented. The action sends generic wording to all active subscriptions for the current user, has a short in-process cooldown, and deactivates subscriptions when a push service definitively reports 404 or 410. It does not write delivery records.

**Phase 2.10C Checkpoint 2 production implementation:** `20260910170000_add_buy_later_reminder_claims.sql`, `20260910180000_fix_buy_later_reminder_claim_ambiguity.sql`, and `20260910190000_fix_buy_later_reminder_conflict_target.sql` provide an atomic, service-only claim function and are applied to the linked remote project. Supabase Cron runs at minute `:05` each hour, reads the stable scheduler route and bearer authorization from Vault, and invokes the protected Node.js Vercel route. The route processes up to 1,000 selected users, three items per user, and 250 pushes per invocation. It applies each stored IANA timezone at or after 09:00 local, uses a rollout-date gate to avoid an old backlog, and records one durable `sent`, `failed`, or `revoked` result for each `(item, reconsideration date, subscription, web_push)` identity. Failed claimed deliveries are not retried automatically; definitive 404/410 provider responses deactivate stale subscriptions. Generic wording is default, item names require opt-in, and payloads use safe Buy Later deep links. Production validation covered iPhone PWA subscription, manual push, scheduler authentication, remote permissions, eligible and ineligible claims, idempotency, distinct subscriptions, a real automatic delivery, and a duplicate-free immediate second invocation. There is no custom notification time, email channel, notification history UI, badge, retry queue, analytics, digest, or cross-device subscription-management UI.
