...
**Phase 2.10C Checkpoint 2:** a service-only atomic claim function, protected Node.js scheduler route, bounded per-device Web Push delivery, and sent/failed/revoked recording are implemented. `20260910170000_add_buy_later_reminder_claims.sql`, `20260910180000_fix_buy_later_reminder_claim_ambiguity.sql`, and `20260910190000_fix_buy_later_reminder_conflict_target.sql` are applied remotely; the claim RPC passed service-role, idempotency, eligibility, and access-boundary validation. The documented hourly Supabase Cron + `pg_net` job uses Vault-held stable-origin and scheduler-secret values; no credential appears in a migration. Eligibility is stored-timezone local date/time at or after 09:00, with a rollout-date gate that suppresses old-item backlog. Vault/Cron activation, deployment, and real-device automatic-delivery QA remain pending.
