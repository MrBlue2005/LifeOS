...
begin
  ...
  with enabled_preferences as (
    ...
    join pg_catalog.pg_timezone_names as timezone on timezone.name = preference.timezone
    ...
    select *
    from per_user_items
    where item_rank <= max_items_per_user
    order by user_id, reconsider_at, item_id, subscription_id
    limit max_pushes
  ), claimed as (
